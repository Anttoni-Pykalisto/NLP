...
class Preprocessing:
	def __init__(self, input=None,vocabulary_input=None, vocabulary_min_frequency=1, vocabulary_max_size=None,remove_stopwords="on",stemming="on",create_context="on",context_window_size=2,create_batches="on",batch_size=30):
	...
	def preprocess(self):
		...
		for i in range(len(index)):
			finding = Text(index[i], finding_text[i], self.remove_stopwords, self.stemming)
		...
		return [[vocabulary,batch,all_finding],self.input]
